...
arc_gen_STD(DisasCtxt *ctx, TCGv src1, TCGv src2, TCGv dest)
{
    ...
    if (instructionHasRegisterOperandIn (0)) {
        ...
        tcg_gen_setcondi_i32(TCG_COND_EQ, temp_1, temp_6, 1);
        tcg_gen_xori_i32(temp_2, temp_1, 1);
        tcg_gen_andi_i32(temp_2, temp_2, 1);
        tcg_gen_brcond_i32(TCG_COND_EQ, temp_2, arc_true, done_1);
        tcg_gen_movi_i32(tmp, 4294967295);
        gen_set_label(done_1);
        tcg_gen_addi_i32(temp_8, address, 4);
...
arc_gen_NORMH(DisasCtxt *ctx, TCGv src, TCGv dest)
{
    int ret = DISAS_NEXT;
    TCGv psrc = tcg_temp_local_new_i32();
    tcg_gen_andi_i32(psrc, src, 65535);
    tcg_gen_ext16s_tl(psrc, psrc);
    tcg_gen_clrsb_i32(dest, psrc);
    tcg_gen_subi_i32(dest, dest, 16);
    if ((getFFlag () == true)) {
        setZFlag(psrc);
        setNFlag(psrc);
    }
...
arc_gen_FLS(DisasCtxt *ctx, TCGv src, TCGv dest)
{
    ...
    TCGLabel *else_1 = gen_new_label();
    TCGLabel *done_1 = gen_new_label();
    tcg_gen_setcondi_i32(TCG_COND_EQ, temp_1, psrc, 0);
    tcg_gen_xori_i32(temp_2, temp_1, 1);
    tcg_gen_andi_i32(temp_2, temp_2, 1);
    tcg_gen_brcond_i32(TCG_COND_EQ, temp_2, arc_true, else_1);
    ...
    tcg_gen_br(done_1);
    gen_set_label(else_1);
    tcg_gen_movi_i32(temp_5, 32);
    tcg_gen_clz_i32(temp_4, psrc, temp_5);
    tcg_gen_mov_i32(temp_3, temp_4);
    tcg_gen_subfi_i32(dest, 31, temp_3);
    gen_set_label(done_1);
    if ((getFFlag () == true)) {
        setZFlag(psrc);
        setNFlag(psrc);
...
arc_gen_FFS(DisasCtxt *ctx, TCGv src, TCGv dest)
{
    ...
    tcg_gen_andi_i32(temp_2, temp_2, 1);
    tcg_gen_brcond_i32(TCG_COND_EQ, temp_2, arc_true, else_1);
    tcg_gen_movi_i32(dest, 31);
    tcg_gen_br(done_1);
    gen_set_label(else_1);
    tcg_gen_movi_i32(temp_4, 32);
    tcg_gen_ctz_i32(temp_3, psrc, temp_4);
    tcg_gen_mov_i32(dest, temp_3);
    gen_set_label(done_1);
    if ((getFFlag () == true)) {
        setZFlag(psrc);
        setNFlag(psrc);
